...
const FLASHSCORE_URL =
  "https://www.flashscore.dk/fodbold/england/premier-league/kommende/";
...
const matchLimit =
  limitIndex >= 0 ? parseInt(args[limitIndex + 1]) : fetchAll ? 999 : 10;
...
async function getMatchList(page) {
  ...
  try {
    // Try to navigate with a simpler wait condition first
    ...
    if (error.message.includes("ERR_CONNECTION_CLOSED")) {
      console.log(
        "⚠️  Connection blocked by Cloudflare. Waiting 5 seconds and retrying...\n",
      );
  ...
  console.log("⏳ Waiting for page to settle (solve CAPTCHA if needed)...\n");
  ...
  const matches = await page.evaluate(() => {
    ...
    const container = document.querySelector(
      '.leagues--static, .sportName, [class*="leagues"]',
    );
    ...
    for (const el of allDivs) {
      ...
      if (
        className.includes("event__header") ||
        className.includes("event__round")
      ) {
  ...
  for (let i = 0; i < Math.min(5, matches.length); i++) {
    const m = matches[i];
    console.log(
      `    ${i + 1}. ${m.homeTeam} vs ${m.awayTeam} (${m.date || "unknown date"})`,
    );
  }
  if (matches.length > 5) {
...
async function getMatchOdds(browser, match, isDebug = false) {
  ...
  try {
    ...
    try {
      ...
      if (error.message.includes("ERR_CONNECTION_CLOSED")) {
        if (isDebug) console.log("🔍 DEBUG: Connection closed, retrying...");
        await new Promise((resolve) => setTimeout(resolve, 2000));
        await page.goto(oddsUrl, {
          waitUntil: "domcontentloaded",
          timeout: 30000,
    ...
    if (isDebug) {
      console.log(`🔍 DEBUG: Page loaded, waiting for content...`);
    }

    // Wait for odds to load - try waiting for specific elements
    // Give much more time for page to load and for dynamic content to appear
    await new Promise((resolve) => setTimeout(resolve, 5000)); // Wait 5 seconds for page to fully load
    ...
    if (isDebug) {
      ...
      if (
        ...
        console.log("🔍 DEBUG: CAPTCHA/Cloudflare check detected!");
      }
    }

    try {
      await page.waitForSelector('.ui-table__row, .oddsCell, [class*="odds"]', {
        timeout: 10000,
      });
      if (isDebug) console.log("🔍 DEBUG: Found odds elements!");
    } catch (e) {
      if (isDebug)
        console.log("🔍 DEBUG: No odds elements found, continuing anyway...");
    }
    ...
    const odds = await page.evaluate(() => {
      ...
      const oddsCells = document.querySelectorAll(
        '[class*="oddsCell"] a, [class*="oddsCell"] span',
      );
      const cellValues = [];
    ...
    if (!odds) {
      ...
      if (isDebug) {
        ...
        console.log(
          "🔍 DEBUG: Page kept open for manual inspection. Press Ctrl+C when done.",
        );
...
async function main() {
  ...
  console.log(
    "⚠️ If Cloudflare CAPTCHA appears, solve it manually in the browser window\n",
  );
  ...
  try {
    ...
    await mainPage.evaluateOnNewDocument(() => {
      ...
      // Add other properties to look more like a real browser
      window.chrome = { runtime: {} };
    ...
    console.log("⏳ Opening Flashscore... (you may need to solve CAPTCHA)\n");
    ...
    console.log(
      "💡 Tip: First match will run in DEBUG mode to help diagnose issues\n",
    );
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      while (!odds && attempts < MAX_RETRIES) {
        ...
        odds = await getMatchOdds(browser, match, isFirstMatch);
    ...
    const data = {
      ...
      odds: matchesWithOdds.flatMap((m) => [
        m.odds.home,
        m.odds.draw,
        m.odds.away,
      ]),
    ...
    console.log(
      `  ${fileExists ? "📝 Updated" : "✨ Created"}: ${todayDate}.json (${data.teams.length} matches)`,
    );
