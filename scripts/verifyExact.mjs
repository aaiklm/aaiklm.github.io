/**
 * Exact Verification - Uses SAME RNG as TypeScript code (mulberry32)
 */
...
// EXACT SAME RNG AS TYPESCRIPT CODE (mulberry32)
...
function test(betsResults) {
  ...
  return { roi: c > 0 ? ((w - c) / c) * 100 : 0, profit: w - c, days: d, cost: c, winnings: w };
}

// OPTIMAL PARAMS from deep tuning
...
function runTeamIntelligence(p, seed = 42) {
  return data.map(df => {
    ...
    const matchProbs = selectedIndices.map(matchIdx => {
      if (matchIdx >= df.teams.length || !df.teams[matchIdx]) {
        return [0.4, 0.3, 0.3];
      }
    ...
    while (bets.length < BETS && attempts < BETS * 30) { 
      const pred = matchProbs.map(p => { const x = random(); return x < p[0] ? "1" : x < p[0] + p[1] ? "X" : "2"; }); 
      const k = pred.join(","); 
...
console.log("   EXACT VERIFICATION (using mulberry32 RNG)");
console.log("=".repeat(70));

const optimalResult = test(runTeamIntelligence(OPTIMAL_PARAMS));
const randomResult = test(runRandom());

console.log("\n┌" + "─".repeat(60) + "┐");
console.log("│ STRATEGY                    │   ROI    │ PROFIT  │ COST    │");
console.log("├" + "─".repeat(60) + "┤");
console.log(`│ 🥇 Team Intel (Optimal)     │ ${(optimalResult.roi >= 0 ? "+" : "") + optimalResult.roi.toFixed(2) + "%"} │ ${optimalResult.profit.toFixed(0).padStart(7)} │ ${optimalResult.cost} │`);
console.log(`│ 📊 Random (Baseline)        │ ${(randomResult.roi >= 0 ? "+" : "") + randomResult.roi.toFixed(2) + "%"} │ ${randomResult.profit.toFixed(0).padStart(7)} │ ${randomResult.cost} │`);
console.log("└" + "─".repeat(60) + "┘");

console.log(`\n📈 Improvement: ${(optimalResult.roi - randomResult.roi).toFixed(2)}pp vs Random`);
console.log(`\n📋 Expected UI values:`);
console.log(`   - Total Bets: ${data.length * BETS}`);
console.log(`   - Total Cost: ${optimalResult.cost}`);
console.log(`   - Winnings: ${optimalResult.winnings.toFixed(1)}`);
console.log(`   - Profit: ${optimalResult.profit.toFixed(1)}`);
console.log(`   - ROI: ${optimalResult.roi.toFixed(2)}%`);
